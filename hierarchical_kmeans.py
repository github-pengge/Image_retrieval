...
class hierarchical_kmeans(object):
	...
	def cluster(self, data, des2im_id, iteration=1):
		data = np.asarray(data)
		cluster = self.clusters
		current_node = self.root
		idx = np.arange(data.shape[0])
		print('Doing hierarchical kmeans...')
		self._cluster(data, idx, des2im_id, current_node, cluster, iteration=iteration)
		print('Done! Actual total number of clusters is %d.' % self.total_k)

	def _cluster(self, data, idx, des2im_id, current_node, cluster_node, iteration=1):
		if cluster_node.is_leaf_node():
			self.total_k += 1
			hist = np.unique(des2im_id[idx], return_counts=True)
			current_node.set_additional_data(list(zip(hist[0], hist[1])))
			return
		...
		for i, c in enumerate(cluster_node):
			...
			self._cluster(data, idx_i, des2im_id, branch, c, iteration=iteration)
